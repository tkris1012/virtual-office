...
**「匿名」** を選んで **有効** にして保存。
（このアプリはユーザーに自動で匿名アカウントを発行し、その uid をアバターIDに使います）

### 3. セキュリティルール

Realtime Database → **ルール** タブに、`database.rules.json` の中身を貼って公開。
**匿名認証済みユーザーだけ**が読み書きでき、**自分のアバターは自分しか動かせない**ルールです。
（手順 2 の匿名認証を有効化していないと、アプリは起動時にサインインで止まります）
...
- **匿名認証済み（`auth != null`）のみ**読み書き可能 → URLを知っただけの未認証アクセスは拒否。
...
さらに堅くするなら: 社内ドメインの Google ログイン（匿名→正式アカウント連携）、room ごとの入室制限、`.validate` による値の形式チェックなど。
