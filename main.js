...
function setupControls(media) {
  ...
  const sendFromQuickPopover = async () => {
    ...
    if (ok) {
      ...
      if (!chatManualOpenOnly && !chatPanelOpen) openChatPanel();
  ...
  chatQuickInput.addEventListener("input", () => autoResizeTextarea(chatQuickInput));
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Tab" && (!chatQuickPopover.hidden || chatPanelOpen)) {
      event.preventDefault();
      toggleChatPanel(); // ポップオーバーは開いたまま、履歴パネルだけを開閉する
      return;
    }
...
addEventListener("keydown", (e) => {
  ...
  if (startSpriteAttack(me, performance.now())) e.preventDefault();
});
...
const SPRITE_ATTACK_CELL = 128;
const SPRITE_ATTACK_COLS = 6;
const SPRITE_ATTACK_FRAME_MS = 100;
const SPRITE_ATTACK_FEET_ROW = 93;
const SKELETON_ATTACK_CHARACTER = Object.freeze({
  folder: "lpc_スケルトン_20260714/custom/slash_128",
  layers: [
    "009 smash__axe_.png",
...
function getSkeletonAttackCanvas() {
  const state = loadSpriteCharacter("skeleton-attack-slash", SKELETON_ATTACK_CHARACTER);
  return state.ready ? state.canvas : null;
}

// 初回のLキー入力時に読み込み待ちでモーションが欠けないよう、起動時から非同期で準備する。
getSkeletonAttackCanvas();

function attackFacingVector(facing) {
...
function isPlayerInSkeletonAttackRange(attacker, target, facing) {
  if (areaOfPlayer(target) !== areaOfPlayer(attacker)) return false;
  const direction = attackFacingVector(facing);
  ...
  return (
    forward >= -AVATAR_R &&
    forward <= SKELETON_ATTACK_RANGE &&
    side <= SKELETON_ATTACK_HALF_WIDTH
  );
}

function skeletonAttackTargetMap(attacker, facing) {
  const targets = {};
  for (const [id, player] of Object.entries(others)) {
    if (isPlayerInSkeletonAttackRange(attacker, player, facing)) targets[id] = true;
  }
  return targets;
}

function publishSkeletonAttack(attacker, facing) {
  if (!meRef || attacker !== me) return;
  const action = {
    id: createEventId(),
    type: SKELETON_ATTACK_ACTION_TYPE,
    facing,
    startedAt: serverTimestamp(),
    targets: skeletonAttackTargetMap(attacker, facing),
  };
  update(meRef, { "runtime/action": action }).catch((error) => {
    console.warn("攻撃アクションの同期に失敗:", error);
  });
}

function startSpriteAttack(p, now) {
  if (!isSkeletonSprite(p) || p.spriteAttacking) return false;
  const facing = SPRITE_ROW_BY_FACING[p.facing] === undefined ? "down" : p.facing;
  p.facing = facing;
  p.spriteAttacking = true;
  p.spriteAttackFrame = 0;
  p._spriteAttackStartedAt = now;
  publishSkeletonAttack(p, facing);
  return true;
}
...
function syncRemoteCombatAction(attackerId, player, serverNow) {
  const action = player?.runtime?.action;
  if (!action || typeof action !== "object") return;
  if (typeof action.id !== "string" || !action.id || action.id.length > 64) return;
  if (action.type !== SKELETON_ATTACK_ACTION_TYPE || !Number.isFinite(action.startedAt)) return;
  if (SPRITE_ROW_BY_FACING[action.facing] === undefined) return;

  const elapsed = serverNow - action.startedAt;
  if (elapsed < -PRESENCE_FUTURE_TOLERANCE_MS) return;

  if (
    elapsed < SPRITE_ATTACK_COLS * SPRITE_ATTACK_FRAME_MS &&
    player._spriteAttackActionId !== action.id &&
    isSkeletonSprite(player)
  ) {
    player.facing = action.facing;
    player.spriteAttacking = true;
    player.spriteAttackFrame = Math.max(0, Math.floor(elapsed / SPRITE_ATTACK_FRAME_MS));
    player._spriteAttackStartedAt = performance.now() - Math.max(0, elapsed);
    player._spriteAttackActionId = action.id;
  }

  if (lastProcessedCombatActionIds.get(attackerId) === action.id) return;
  lastProcessedCombatActionIds.set(attackerId, action.id);
  if (
    elapsed <= SKELETON_ATTACK_DAMAGE_ACCEPT_MS &&
    isSkeletonSprite(player) &&
    action.targets &&
...
function updateSpriteAttack(p, now) {
  if (!p.spriteAttacking) return false;
  if (!isSkeletonSprite(p)) {
    p.spriteAttacking = false;
    p.spriteAttackFrame = 0;
    return false;
  }

  const elapsed = now - (p._spriteAttackStartedAt ?? now);
  const frame = Math.floor(elapsed / SPRITE_ATTACK_FRAME_MS);
  if (frame >= SPRITE_ATTACK_COLS) {
    p.spriteAttacking = false;
    p.spriteAttackFrame = 0;
    return false;
  }
  p.spriteAttackFrame = frame;
  return true;
}
...
function carrySpriteAnimationState(newPlayer, oldPlayer) {
  ...
  newPlayer.spriteAttacking = oldPlayer.spriteAttacking;
  newPlayer.spriteAttackFrame = oldPlayer.spriteAttackFrame;
  newPlayer._spriteAttackStartedAt = oldPlayer._spriteAttackStartedAt;
  newPlayer._spriteAttackActionId = oldPlayer._spriteAttackActionId;
...
function updateSpriteAnimation(p, now) {
  if (updateSpriteAttack(p, now)) return;
  if (!p.spriteMoving) {
    p.spriteFrame = 0;
    return;
  }
...
function drawSpriteAvatar(p) {
  const attackSheet = p.spriteAttacking && isSkeletonSprite(p) ? getSkeletonAttackCanvas() : null;
  const sheet = attackSheet || getPlayerSpriteCanvas(p);
  if (!sheet) return false;
  const row = SPRITE_ROW_BY_FACING[p.facing || "down"];
  const isAttacking = !!attackSheet;
  const cell = isAttacking ? SPRITE_ATTACK_CELL : SPRITE_CELL;
  const col = isAttacking
    ? (p.spriteAttackFrame || 0) % SPRITE_ATTACK_COLS
    : (p.spriteFrame || 0) % SPRITE_COLS;
  const destH = SPRITE_DRAW_HEIGHT;
  const sourceScale = destH / SPRITE_CELL;
  const destCellSize = cell * sourceScale;
  const destX = -destCellSize / 2;
  const destY = isAttacking
    ? -(SPRITE_ATTACK_FEET_ROW + 1) * sourceScale - SPRITE_FEET_VISUAL_INSET
    : -destH;
