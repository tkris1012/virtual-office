...
- 背景ぼかし / バーチャル背景（MediaPipe Selfie Segmentation）。カメラボタン右の矢印(∨)から開く
...
### バーチャル背景の永続化（2026-07-07 実装済み・Issue #7）
- 背景モード（none/blur/image）・ぼかし強さ・背景画像を保存し、次回入室時に自動で再適用する
- 保存先は `backgroundSettings/{uid}`（`users/{uid}` とは別ノード。プロフィール保存が `set()` で `users/{uid}` を丸ごと上書きするため、同じ場所に置くと消えてしまうことに注意）
- **`.read`/`.write` とも本人のみ**（他ユーザーへ配信しない個人設定のため。アバターアイコンより広めの960px/JPEG品質0.82で圧縮＝動画背景として十分な解像度）
- 保存タイミング: モード変更時／ぼかしスライダーは`change`（ドラッグ終了時のみ、連続書き込み防止）／画像アップロード時
- プロフィールと同じ「クラウド／`localStorage`控えの新しい方を採用」方式。復元失敗時（segmentation読込失敗・画像破損等）はトーストで通知し `none` にフォールバック
- 実装: `saveBackgroundPreference()`/`loadBackgroundPreference()`/`compressBackgroundImage()` @ `main.js`（`setupControls()` 内の背景セクション）
