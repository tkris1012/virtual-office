...
- **エリア通話**（2026-06-16）: `ZONES`（会議室/ラウンジ/集中スペース/受付）。同じ部屋にいる人同士は距離無関係に通話＋フル音量。接続判定は `updateConnections()`。屋外は従来の近接。部屋枠を描画し自室は強調表示。
- **画面共有**: `media.js` の `startScreenShare()`（getDisplayMedia→canvasに描画）。送出トラックを差し替えないので全ピアに自動反映・再ネゴ不要。操作バーの「🖥️ 画面共有」でトグル。
- **全体アナウンス**: 「📢 アナウンス」で presence の `announcing` を立てる→全員が発信者と接続＋上部バナー表示。
- **集合**: 「👥 集合」でオンライン一覧から選択→自分ノードに `summon{targets,x,y,ts}` 書込→対象が自動でその地点へワープ＋トースト。RTDBルール変更不要（自分ノードのみ書込）。
