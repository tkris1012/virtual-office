...
**最終更新: 2026-06-19（カスタムアバター＋サインイン認証を実装）**
...
- 構成: GitHub Pages（静的）+ Firebase Realtime Database（位置同期＆WebRTCシグナリング）+ Firebase Auth + Firebase Storage（アバター画像）+ WebRTC P2P。無料枠中心。
...
- **画像アップロード**: PNG/JPG → **円形クロップ**（`openCrop()` @ `main.js`・Canvasのみ／ドラッグ＋ズーム）→ Firebase Storage `avatars/{uid}/icon.png` にアップロード
- **× で閉じると即保存＆反映**（`closeConsole()` → `saveProfile()`）。在席ノードにも書くので他参加者のマップへ即反映
- マップ上のアバター描画は `drawAvatar()` を円形クリップ＋画像/絵文字描画に刷新。アップロード画像は `avatarImgCache` でキャッシュ
- 保存先: RTDB `users/{uid}/`（displayName, iconType, iconId, iconUrl）。在席 `rooms/{room}/players/{uid}` にも icon* を複製
...
3. **Storage**: 「始める」で有効化し、Rules を設定（README「3. Storage を有効化」参照）
4. **Realtime Database → ルール**: `database.rules.json`（`users/{uid}` ルール追加済み）を再公開
